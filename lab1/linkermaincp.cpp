/* Tokens parsed line by line, can't save it all into a tokBuffer */
#include <iostream>
#include <vector>
#include <fstream>
#include <cstring>
...
using namespace std;
/* QUESTIONS */
//Check about input saved up in tokBuffer
//Check about the address in def exceeding module size
//**********************************************************
...
int passOne(const char *input) {
    ...
    tokBuffer = tokenParser(input);  // why not pass in &input?
    while (j < tokBuffer.size()) {
        baseAddMod.insert(pair<int, int> (moduleNum, baseAddressMod));  
        int defCount = readInt(tokBuffer[j].token);
        if (defCount == -1) {
        ...
        for (int i = 0; i < defCount; i++) {
            ...
            j++;
            buf.push_back( {sym, temp} );   // use sym AND temp?
...
/*if (store == "" && lineNum != 1) { //if as a consequence of newline, store is just empty string
        for (int j = listLeng.size()-1; j >= 0; j--) {
            if (listLeng.at(j) != 0) {
                res.linenum = j+1;
                res.offset = listLeng.at(j)+1;
                return res;
            }
        }
...
/*    map<string, int>:: iterator it;
    ...
    for (it = symbolandmod.begin(); it != symbolandmod.end(); it++) {
        for (int i = 0; i < symdefnotused.size(); i++) {
            if (it->first == symdefnotused.at(i)) {
                break;
            } else if ((i == symdefnotused.size() - 1) && (it->first != symdefnotused.at(i))) {
                printf("Warning: Module %d: %s was defined but never used\n",it->second,it->first.c_str());
                break;
            }
...
vector<TokOffLine> tokenParser(const char *s) {
    ...
    /*  OR I COULD HAVE DONE THIS TO PUSH TO PACKET
    for (int count = 0; count < tokVec.size(); count++) {
        packet.push_back( {tokVec[count], rVec[count], cVec[count] }  );
    }
    */
  // cout << "The final position offset in the file is " << listLeng.back()+1 << endl;
   /*
    ...
    for (int i = 0; i < packet.size(); i++) {
        cout << "Token: " << packet[i].token << endl;
        cout << "LineNum: " << packet[i].lineNumber << endl;
        cout << "LineOffset: "<<  packet[i].lineOffset << endl;
    }
    cout << endl;
    return packet;
}*/
...
vector<TokOffLine> tokenParser(const char *s) {
    ...
    while (getline(file,store)) {
        ...
        nextTokPos = 0;
        listLeng.push_back(store.length());
        const char *src = store.c_str();
        ptr = new char[store.length() + 1];  //should delete after
        strcpy(ptr,src);
        char *dest = strtok(ptr," \t");
        while (dest != NULL) {
            string s = std::string(dest);
            tokVec.push_back(s);  //convert char* to sring using constructor
            rVec.push_back(lineNum);
            lineOffset = s.find(s,nextTokPos);  //returns index of first encounter of token
            lineOffset++;
            cVec.push_back(lineOffset);
            packet.push_back( {s,lineNum,lineOffset} );  // use -std=c++11
            cout << "Token " << s << " is at line " << lineNum << " with offset position " << lineOffset << endl;
            nextTokPos = lineOffset + s.length();  //it will be at least after the pos of token
            dest = strtok(NULL," \t");
        }
        delete [] ptr;
